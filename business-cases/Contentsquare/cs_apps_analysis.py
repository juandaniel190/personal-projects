...
# --- MAU data for P3 slide: all months from CSV (same as fig_05) ---
mau_table_rows = []
for _, row in mau.iterrows():
    month_dt = pd.to_datetime(row['MONTH'])
    month_label = month_dt.strftime('%b %Y')
    n_users = int(row['n_unique_users'])
    total_sess = int(row['total_sessions'])
    sess_per_user = round(total_sess / n_users, 1) if n_users else 0
    mau_table_rows.append({
        'month': month_label,
...
# MAU table for P3 slide (Monthly Active Users — same as fig_05)
js_mau_rows = '[' + ','.join(
    f"{{month:{_js_val(r['month'])},unique_users:{r['unique_users']},total_sessions:{r['total_sessions']},sessions_per_user:{r['sessions_per_user']}}}"
    for r in mau_table_rows
) + ']'
mau_data_js = f"""      // MAU_DATA_START (generated by cs_apps_analysis.py — do not edit)
      const MAU_DATA = {{ rows: {js_mau_rows} }};
      // MAU_DATA_END"""
...
pattern_mau = re.compile(r'      // MAU_DATA_START[\s\S]*?      // MAU_DATA_END', re.MULTILINE)
if pattern_mau.search(html_content):
    html_content = pattern_mau.sub(mau_data_js.strip(), html_content)
    print("MAU data written to cs_apps_presentation.html (Slide4 P3).")
else:
    print("Warning: MAU_DATA_START / MAU_DATA_END placeholder not found in HTML; skip injection.")
