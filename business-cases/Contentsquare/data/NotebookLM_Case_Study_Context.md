...
**H4: Implementation Bottleneck — CS Apps Implementation Status (latest snapshot):**
...
**H6: Shallow Module Engagement** — 1 in 4 sessions lands on the Homepage. Users who reach Zoning, Workspace, or Error Analysis show higher engagement; this is a depth-of-usage problem.
...
> **Important context on UFR calculation:** For each fiscal quarter, UFR equals the TOTAL_UFR in the **last month** of that quarter only. You do not sum across months within a quarter because accounts that appeared in prior months already renewed. For example, if November shows $8M and December shows $8M, the Q4 UFR is only the December value ($8M), not $16M summed.
...
**New Health Score Scenario — Q4-2023 Account Health Breakdown** (using the proposed three-pillar composite health score detailed in the Health KPI slide):
...
- **NULL Implementation Status (HIGH):** 29% of accounts (82 accounts), 518 rows untracked — systemic pipeline issue, blocks the entire adoption funnel analysis.
- **Contract Date Array Problem (MEDIUM):** CONTRACT_START_DATE and CONTRACT_END_DATE store comma-delimited arrays of multiple dates per account — 60% of accounts have 2 or more contract start dates because they hold multiple active contracts simultaneously (CS Digital, CS Apps, annual renewals stacked). This makes time-to-live calculations impossible without parsing logic to isolate which date maps to the CS Apps contract. Blocks cohort analysis and TTL calculations.
- **AVG_WAU_GLOBAL = 0 (CRITICAL):** 156 rows across 49 accounts — breaks the health metric for approximately 27% of the base.
...
- Formula: `Cert / (ACV × alpha)`
- Where `alpha = 1/100,000` (adoption rate per dollar of ACV)
...
- Formula: `ACV / (WAU + Sense Q/wk) × beta`
- Where `beta = 1/500` (engagement sensitivity)
...
**Illustrative Examples (Dummy Data)** — Assumed constants: `alpha = 1/100,000`; `beta = 1/500`; Expected implementation window = 90 days:
...
|---|---|---|
| alpha: adoption rate per $ ACV | 1/100,000 | Distribution of (Cert Users / ACV) across live accounts |
| beta: engagement sensitivity | 1/500 | Distribution of ACV / (WAU + Sense Q) ratios; calibrate so P50 account scores ~50 |
...
2. *"You identified three data quality issues. How would you prioritize fixing them, and how would you communicate the urgency to engineering?"*
   She wants to see stakeholder management. Daniel should rank: AVG_WAU_GLOBAL = 0 is CRITICAL (breaks 27% of health scores for 49 accounts), NULL Implementation Status is HIGH (blocks the entire adoption analysis), Contract Date Array is MEDIUM (the field exists but stores multiple comma-separated dates per account — 60% of accounts have 2 or more — requiring disambiguation logic before TTL cohort analysis can be built).
...
5. *"What's missing from this analysis that concerns you the most?"*
   Intellectual honesty test. Daniel should mention: time-to-go-live cohort analysis is blocked because the contract date field stores arrays of multiple dates per account and needs parsing before cohorts can be built; competitive win/loss data; and the inability to validate the health score against actual renewal outcomes since Q4 hasn't happened yet.
...
2. *"Your health score uses constants alpha = 1/100,000 and beta = 1/500. How did you arrive at these? What happens to the score distribution if alpha is off by 2x?"*
   Sensitivity analysis. Daniel should say: "These are initial estimates calibrated so that a $100K account with 1 certified user scores P2=100, which felt reasonable as a starting point. If alpha is 2x too high, the P2 distribution shifts left and more accounts appear adoption-unhealthy. That's why I included a calibration table — these constants need to be validated against the distribution of certified users across live accounts."
...
3. *"You say sessions per user fell from 14.5 to 10. Is that a real engagement decline or just a base-rate effect from adding low-engagement new users?"*
   Cohort analysis question. Daniel should acknowledge: "This is likely a mix of both. To isolate the effect, I would need cohort-level analysis — segment users by their first active month and track session frequency over time. However, the CONTRACT_START_DATE field stores a comma-delimited array of multiple dates per account (60% of accounts have 2 or more start dates from multiple simultaneous contracts), so I cannot cleanly assign a single cohort date without disambiguation logic to isolate the CS Apps-specific contract. I flagged this as a data quality issue."
...
- **Time-to-Live (TTL):** The number of days from contract start to "Lived" implementation status. This calculation is blocked by the contract date array problem — CONTRACT_START_DATE stores a comma-delimited list of multiple dates per account (60% of accounts have 2+ start dates from multiple simultaneous contracts), requiring parsing and disambiguation before TTL can be computed.
...
- **NULL Rate / Missing Data Rate:** Percentage of records with missing values. Daniel flagged 29% NULL rate on implementation status.
- **Contract Date Array Problem:** A data quality issue where a field stores multiple comma-delimited values per record instead of a single scalar. The CONTRACT_START_DATE field has no NULLs but is analytically unusable for TTL calculations because 60% of accounts have 2 or more dates reflecting multiple simultaneous contracts. Disambiguation logic is required before the field can be used for cohort analysis.
...
- **Sensitivity Analysis:** Testing how results change when assumptions change (e.g., what if alpha = 1/50,000 instead of 1/100,000?).
...
- "I would validate this with a cohort analysis, but the contract date field stores a comma-delimited array of multiple dates per account — before I can compute time-to-live, I need parsing logic to isolate which date maps to the CS Apps contract specifically."
